...
function EWD_UWPM_New_Comment_On_Post($comment_ID, $comment_approved = 0) {
	$Send_On_Actions = get_option("EWD_UWPM_Send_On_Actions");
	if (!is_array($Send_On_Actions)) {$Send_On_Actions = array();}
...
add_action('comment_post', 'EWD_UWPM_New_Comment_On_Post', 10, 2);
...
function EWD_UWPM_Send_On_WC_X_Time_Since_Cart_Abandoned() {
	...
	foreach ($Send_On_Actions as $Send_On_Action) {
		if ($Send_On_Action['Action_Type'] == "WC_X_Time_Since_Cart_Abandoned") {
			...
			foreach ($Users as $User) {
				$includes_match =  ( $Send_On_Action['Includes'] == 'Any' );
				if( !$includes_match ){
					$product_id = false;
					$product_cat = false;
					if( substr( $Send_On_Action['Includes'], 0, 1 ) == 'P' )
						$product_id = intval( substr( $Send_On_Action['Includes'], 2 ) );
					if( substr( $Send_On_Action['Includes'], 0, 1 ) == 'C' )
						$product_cat = intval( substr( $Send_On_Action['Includes'], 2 ) );

					$user_cart = $wpdb->get_var(
						$wpdb->prepare(
							"
							...
							foreach ( $user_cart['cart'] as $cart_item ) {
								if( $product_id ){
									...
									foreach( $product_categories as $category ){
										if( $category->term_id == $product_cat ){
											$includes_match = true;
										}
									}
								}
